...
{
	struct _tai_hook_user *cur = (struct _tai_hook_user *)hookRef;
	struct _tai_hook_user *next = (struct _tai_hook_user *)cur->next;
	typedef void *(*FUN_811fe7a4_t)(uint32_t, uint32_t, uint32_t, uint32_t,
		uint32_t, uint32_t, uint32_t, uint32_t);

	*(uint8_t *)(param_3 + 0xb) = *(*(uint8_t **)(param_1 + 0x90) + 0x3a); // pedanticError
	*(uint8_t *)(param_7 + 0x14) = 1;									   // Extension support

	/* GCC 15 diagnoses TAI_CONTINUE's old empty-parameter function cast. */
	if (next == NULL)
		return ((FUN_811fe7a4_t)cur->old)(param_1, param_2, param_3, param_4,
			param_5, param_6, param_7, param_8);
	return ((FUN_811fe7a4_t)next->func)(param_1, param_2, param_3, param_4,
		param_5, param_6, param_7, param_8);
}
