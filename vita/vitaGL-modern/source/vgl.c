...
uint8_t *vgl_reserve_data_pool(uint32_t size) {
	...
	if (!circular_data_pool[vgl_circular_idx] ||
		!circular_data_pool_ptr[vgl_circular_idx] ||
		!circular_data_pool_limit[vgl_circular_idx]) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_alloc_failures++;
		vgl_log("[VGL-DIAG] circular pool unavailable: index=%d bytes=%u free_ram=%u free_vram=%u\n",
			vgl_circular_idx, size,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM));
#endif
		return NULL;
	}
	uint8_t *pool = circular_data_pool[vgl_circular_idx];
	uint8_t *pool_ptr = circular_data_pool_ptr[vgl_circular_idx];
	uint8_t *pool_limit = circular_data_pool_limit[vgl_circular_idx];
	uint8_t *res = NULL;
	uintptr_t pool_addr = (uintptr_t)pool;
	uintptr_t pool_ptr_addr = (uintptr_t)pool_ptr;
	uintptr_t pool_limit_addr = (uintptr_t)pool_limit;
	/* Do not advance a C pointer past the allocation just to discover an
	 * overrun.  NUTS can submit enough texture/vertex staging data in one
	 * frame to exercise this path, and the old code made that undefined
	 * pointer arithmetic a plausible source of Vita3K access violations. */
	if (pool_ptr_addr >= pool_addr && pool_ptr_addr <= pool_limit_addr &&
		size <= (uint32_t)(pool_limit_addr - pool_ptr_addr)) {
		res = pool_ptr;
		circular_data_pool_ptr[vgl_circular_idx] = pool_ptr + size;
	} else {
		circular_data_pool_ptr[vgl_circular_idx] = pool_limit;
	}
#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_frame_circular_requested += size;
	{
		uint32_t used = (uint32_t)((uintptr_t)circular_data_pool_ptr[vgl_circular_idx] - pool_addr);
		if (used > vgl_diag_frame_circular_peak)
			vgl_diag_frame_circular_peak = used;
	}
#endif
	if (!res) {
	#ifdef LOG_ERRORS
		vgl_log("%s:%d Circular pool exhausted on buffer %d: requested=%u free=%u. Trying standalone mapped memory.\n",
			__FILE__, __LINE__, vgl_circular_idx, size,
			pool_ptr_addr <= pool_limit_addr ? (uint32_t)(pool_limit_addr - pool_ptr_addr) : 0);
	#endif
		res = (uint8_t *)gpu_alloc_mapped_for_cpu(size);
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_circular_spills++;
		vgl_diag_frame_circular_spill_bytes += size;
#endif
#ifdef LOG_ERRORS
		if (!res) {
...
#else
	if (!circular_data_pool || !circular_data_pool_ptr || !circular_data_pool_limit) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_alloc_failures++;
		vgl_log("[VGL-DIAG] circular pool unavailable: bytes=%u free_ram=%u free_vram=%u\n",
			size,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM));
#endif
	...
	uint8_t *res;
	if (size <= (uint32_t)(circular_data_pool_limit - circular_data_pool_ptr)) {
		res = circular_data_pool_ptr;
		circular_data_pool_ptr += size;
	} else {
		/* Keep the speed-hack path bounded too.  It intentionally reuses the
		 * start of the pool after a spill, but a request larger than the whole
		 * pool must be serviced by a standalone allocation. */
		res = (uint8_t *)gpu_alloc_mapped_for_cpu(size);
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_circular_spills++;
		vgl_diag_frame_circular_spill_bytes += size;
...
GLboolean vglInitWithCustomSizes(int pool_size, int width, int height, int ram_pool_size, int cdram_pool_size, int phycont_pool_size, int cdlg_pool_size, SceGxmMultisampleMode msaa) {
	...
	for (int i = 0; i < gxm_display_buffer_count; i++) {
		const uint32_t pool_size = circular_data_pool_size / gxm_display_buffer_count;
		circular_data_pool[i] = gpu_alloc_mapped_for_cpu(pool_size);
#ifdef LOG_ERRORS
		if (circular_data_pool[i] && vgl_mem_get_type_by_addr(circular_data_pool[i]) == VGL_MEM_VRAM) {
			vgl_log("%s:%d %s: Circular pool #%d spilled into VRAM. This might negatively impact performance.\n", __FILE__, __LINE__, __func__, i);
		}
#endif
		if (circular_data_pool[i]) {
			circular_data_pool_ptr[i] = circular_data_pool[i];
			circular_data_pool_limit[i] = circular_data_pool[i] + pool_size;
		} else {
			/* Do not form a limit pointer from NULL.  On Vita this can hide an
			 * allocation failure until the first large map, where the resulting
			 * pointer arithmetic becomes an access violation. */
			circular_data_pool_ptr[i] = NULL;
			circular_data_pool_limit[i] = NULL;
#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_frame_alloc_failures++;
			vgl_log("[VGL-DIAG] circular pool allocation failed: index=%d bytes=%u free_ram=%u free_vram=%u\n",
				i, pool_size,
				(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM),
				(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM));
#endif
...
#ifdef LOG_ERRORS
	if (circular_data_pool && vgl_mem_get_type_by_addr(circular_data_pool) == VGL_MEM_VRAM) {
		vgl_log("%s:%d %s: Circular pool spilled into VRAM. This might negatively impact performance.\n", __FILE__, __LINE__, __func__);
	}
#endif
	if (circular_data_pool) {
		circular_data_pool_ptr = circular_data_pool;
		circular_data_pool_limit = circular_data_pool + circular_data_pool_size;
	} else {
		circular_data_pool_ptr = NULL;
		circular_data_pool_limit = NULL;
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_alloc_failures++;
		vgl_log("[VGL-DIAG] circular pool allocation failed: bytes=%u free_ram=%u free_vram=%u\n",
			(uint32_t)circular_data_pool_size,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM));
#endif
	}
