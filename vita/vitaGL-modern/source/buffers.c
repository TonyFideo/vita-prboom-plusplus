...
static inline void vgl_report_pointer_alloc_failure(const char *stage, size_t size) {
#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_frame_alloc_failures++;
	vgl_log("[VGL-DIAG] legacy array allocation failed: stage=%s bytes=%u free_ram=%u free_vram=%u\n",
		stage, (uint32_t)size,
...
void reset_vao(vao *v) {
	...
	v->vertex_attrib_pool_limit = v->vertex_attrib_pool ?
		(float *)((uint8_t *)v->vertex_attrib_pool + circular_pool_size) : NULL;

	// Init generic vertex attrib arrays
	vao *vao_bkp = cur_vao;
	cur_vao = v;
	for (int i = 0; i < VERTEX_ATTRIBS_NUM; i++) {
		v->vertex_attrib_value[i] = v->vertex_attrib_pool ? reserve_attrib_pool(4) : NULL;
...
void reset_queries() {
	...
	if (gxm_context && queries_buffer)
		sceGxmSetVisibilityBuffer(gxm_context, queries_buffer, MAX_QUERIES_NUM * sizeof(uint32_t));
#ifdef VITA_GL_DIAGNOSTICS
	if (!queries_buffer) {
		vgl_diag_frame_alloc_failures++;
		vgl_log("[VGL-DIAG] visibility buffer allocation failed: bytes=%u free_ram=%u free_vram=%u\n",
			(uint32_t)(MAX_QUERIES_NUM * 4 * sizeof(uint32_t)),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM));
	}
#endif
...
void vglVertexPointer(GLint size, GLenum type, GLsizei stride, GLuint count, const GLvoid *pointer) {
	...
	if (!vertex_object || !pointer) {
		if (!vertex_object)
			vgl_report_pointer_alloc_failure("vertex", count * streams->stride);
		return;
	}
...
void vglColorPointer(GLint size, GLenum type, GLsizei stride, GLuint count, const GLvoid *pointer) {
	...
	if (!color_object || !pointer) {
		if (!color_object)
			vgl_report_pointer_alloc_failure("color", count * streams->stride);
		return;
	}
...
void vglTexCoordPointer(GLint size, GLenum type, GLsizei stride, GLuint count, const GLvoid *pointer) {
	...
	if (!texture_object || !pointer) {
		if (!texture_object)
			vgl_report_pointer_alloc_failure("texcoord", count * streams->stride);
		return;
	}
...
void vglIndexPointer(GLenum type, GLsizei stride, GLuint count, const GLvoid *pointer) {
	...
	if (!index_object || !pointer) {
		if (!index_object)
			vgl_report_pointer_alloc_failure("index", count * bpe);
		return;
	}
	if (stride == 0)
