...
GLboolean vgl_gxm_scene_active = GL_FALSE;

#ifdef VITA_GL_DIAGNOSTICS
uint32_t vgl_diag_frame_serial = 0;
uint32_t vgl_diag_frame_draws = 0;
uint32_t vgl_diag_frame_vertices = 0;
uint32_t vgl_diag_frame_legacy_bytes = 0;
uint32_t vgl_diag_frame_legacy_peak = 0;
uint32_t vgl_diag_frame_legacy_overflows = 0;
uint32_t vgl_diag_frame_dropped_draws = 0;
uint32_t vgl_diag_frame_gxm_errors = 0;
uint32_t vgl_diag_frame_last_gxm_error = 0;
uint32_t vgl_diag_frame_stream_errors = 0;
uint32_t vgl_diag_frame_last_stream_error = 0;
uint32_t vgl_diag_frame_circular_requested = 0;
uint32_t vgl_diag_frame_circular_peak = 0;
uint32_t vgl_diag_frame_circular_spills = 0;
uint32_t vgl_diag_frame_circular_spill_bytes = 0;
uint32_t vgl_diag_frame_alloc_failures = 0;
uint32_t vgl_diag_frame_scene_resets = 0;
uintptr_t vgl_diag_legacy_pool_base = 0;
uint32_t vgl_diag_legacy_pool_size = 0;
uint32_t vgl_diag_legacy_pool_global_peak = 0;
#endif
...
static inline __attribute__((always_inline)) void scene_end(void) {
	...
	query_fence.value++;
	int r = sceGxmEndScene(gxm_context, NULL, &query_fence);
#ifdef VITA_GL_DIAGNOSTICS
	if (r) {
		vgl_diag_frame_gxm_errors++;
		vgl_diag_frame_last_gxm_error = (uint32_t)r;
		vgl_log("[VGL-DIAG] sceGxmEndScene failed: 0x%08X (%s)\n", (uint32_t)r, get_gxm_error_literal(r));
	}
#endif
	vgl_gxm_scene_active = GL_FALSE;
...
void scene_reset(void) {
	if (in_use_framebuffer != active_write_fb || needs_scene_reset || dirty_framebuffer || dirty_query) {
		...
		if (needs_end_scene) {
			...
			if (legacy_pool_size) {
				...
				if (legacy_pool) {
					...
					legacy_pool_ptr = NULL;
#ifndef SKIP_ERROR_HANDLING
					legacy_pool_end = NULL;
#endif
				}
			}
#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_frame_scene_resets++;
			vgl_diag_legacy_pool_base = (uintptr_t)legacy_pool;
			vgl_diag_legacy_pool_size = legacy_pool ? (uint32_t)legacy_pool_size : 0;
			if (!legacy_pool && legacy_pool_size) {
				vgl_diag_frame_alloc_failures++;
				vgl_log("[VGL-DIAG] legacy pool allocation failed: requested=%u bytes\n", (uint32_t)legacy_pool_size);
			}
#endif
...
#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_frame_alloc_failures++;
			vgl_log("[VGL-DIAG] scene reset aborted: GXM context is NULL\n");
#endif
			return;
		}
		...
		/* Do not touch GXM state after BeginScene rejected the scene.  Keep the
		 * reset pending so the next draw/frame can retry after transient resource
		 * pressure instead of issuing an invalid draw into Vita3K. */
		if (!vgl_gxm_scene_active) {
			needs_scene_reset = GL_TRUE;
			needs_end_scene = GL_FALSE;
			legacy_pool = NULL;
...
#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_frame_alloc_failures++;
			vgl_log("[VGL-DIAG] scene reset aborted after sceGxmBeginScene failure\n");
#endif
			return;
		}
...
void vglSwapBuffers(GLboolean has_commondialog) {
	...
	if ((vgl_framecount % 30) == 0) {
		vgl_profile_log("-----------------------------------------\n");
		vgl_profile_log("Last 30 frames took %ums to be processed.\n", frame_profiler_cnt / 1000);
		vgl_profile_log("%ums spent processing %u fixed-function pipeline non-immediate draw calls.\n", ffp_draw_profiler_cnt / 1000, ffp_draw_cnt);
		vgl_profile_log("%ums spent setting up fixed-function pipeline states.\n", ffp_reload_profiler_cnt / 1000);
		vgl_profile_log("%ums spent processing %u shaders pipeline draw calls.\n", shaders_draw_profiler_cnt / 1000, shaders_draw_cnt);
		vgl_profile_log("%ums spent waiting for GPU to process frames.\n", gpu_stall_cnt / 1000);
		vgl_profile_log("-----------------------------------------\n");
...
#ifdef HAVE_DEBUG_INTERFACE
	if (circular_data_pool[vgl_circular_idx] && circular_data_pool_ptr[vgl_circular_idx])
		...
		vgl_circular_pool_frame_peak = 0;
...
#ifdef LOG_ERRORS
	if (circular_data_pool[vgl_circular_idx] && circular_data_pool_ptr[vgl_circular_idx] &&
		circular_data_pool_limit[vgl_circular_idx] &&
		circular_data_pool_ptr[vgl_circular_idx] > circular_data_pool_limit[vgl_circular_idx]) {
	...
	if (circular_data_pool[vgl_circular_idx])
...
#ifdef VITA_GL_DIAGNOSTICS
	/* Emit one compact record after the scene has been closed.  Keeping this
	 * here means sceGxmEndScene errors are included in the same frame record,
	 * and gives us the exact resource state seen by the next NUTS frame. */
	{
		uint32_t legacy_used = 0;
		if (vgl_diag_legacy_pool_base && legacy_pool_ptr &&
			(uintptr_t)legacy_pool_ptr >= vgl_diag_legacy_pool_base) {
			legacy_used = (uint32_t)((uintptr_t)legacy_pool_ptr - vgl_diag_legacy_pool_base);
		}
		if (legacy_used > vgl_diag_frame_legacy_peak)
			vgl_diag_frame_legacy_peak = legacy_used;
		if (legacy_used > vgl_diag_legacy_pool_global_peak)
			vgl_diag_legacy_pool_global_peak = legacy_used;

		vgl_diag_frame_serial++;
		if (vgl_diag_frame_serial <= 120 ||
			vgl_diag_frame_legacy_overflows ||
			vgl_diag_frame_dropped_draws ||
			vgl_diag_frame_gxm_errors ||
			vgl_diag_frame_stream_errors ||
			vgl_diag_frame_alloc_failures ||
			vgl_diag_frame_circular_spills) {
			vgl_log("[VGL-DIAG] frame=%u display=%u resets=%u draws=%u vertices=%u legacy=%u/%u bytes=%u peak=%u global=%u overflows=%u dropped=%u streams=%u gxm=%u last_gxm=0x%08X alloc_fail=%u circular=%u/%u peak=%u spills=%u spill_bytes=%u free_ram=%u free_vram=%u free_phycont=%u free_cdlg=%u\n",
				vgl_diag_frame_serial, vgl_framecount,
				vgl_diag_frame_scene_resets,
				vgl_diag_frame_draws, vgl_diag_frame_vertices,
				legacy_used, vgl_diag_legacy_pool_size,
				vgl_diag_frame_legacy_bytes,
				vgl_diag_frame_legacy_peak, vgl_diag_legacy_pool_global_peak,
				vgl_diag_frame_legacy_overflows, vgl_diag_frame_dropped_draws,
				vgl_diag_frame_stream_errors, vgl_diag_frame_gxm_errors,
				vgl_diag_frame_last_gxm_error, vgl_diag_frame_alloc_failures,
				vgl_diag_frame_circular_requested, vgl_diag_frame_circular_peak,
				vgl_diag_frame_circular_spills, vgl_diag_frame_circular_spill_bytes,
				(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM),
				(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
				(uint32_t)vgl_mem_get_free_space(VGL_MEM_PHYCONT),
				(uint32_t)vgl_mem_get_free_space(VGL_MEM_BUDGET));
		}

		vgl_diag_frame_draws = 0;
		vgl_diag_frame_vertices = 0;
		vgl_diag_frame_legacy_bytes = 0;
		vgl_diag_frame_legacy_peak = 0;
		vgl_diag_frame_legacy_overflows = 0;
		vgl_diag_frame_dropped_draws = 0;
		vgl_diag_frame_gxm_errors = 0;
		vgl_diag_frame_last_gxm_error = 0;
		vgl_diag_frame_stream_errors = 0;
		vgl_diag_frame_last_stream_error = 0;
		vgl_diag_frame_circular_requested = 0;
		vgl_diag_frame_circular_peak = 0;
		vgl_diag_frame_circular_spills = 0;
		vgl_diag_frame_circular_spill_bytes = 0;
		vgl_diag_frame_alloc_failures = 0;
		vgl_diag_frame_scene_resets = 0;
	}
#endif
