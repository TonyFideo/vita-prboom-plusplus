...
void vgl_mem_init(size_t size_ram, size_t size_cdram, size_t size_phycont, size_t size_cdlg) {
	...
	vgl_log("[VGL-DIAG] memory init requested: ram=%u cdram=%u phycont=%u cdlg=%u cached=%d\n",
		(uint32_t)size_ram, (uint32_t)size_cdram,
		(uint32_t)size_phycont, (uint32_t)size_cdlg, has_cached_mem);
#endif
...
#ifdef VITA_GL_DIAGNOSTICS
	vgl_log("[VGL-DIAG] memory pools: free_ram=%u/%u addr=0x%08X free_vram=%u/%u addr=0x%08X free_phycont=%u/%u addr=0x%08X free_budget=%u/%u addr=0x%08X external=%u addr=0x%08X\n",
		(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM),
		(uint32_t)mempool_size[VGL_MEM_RAM], (uint32_t)(uintptr_t)mempool_addr[VGL_MEM_RAM],
		(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
		(uint32_t)mempool_size[VGL_MEM_VRAM], (uint32_t)(uintptr_t)mempool_addr[VGL_MEM_VRAM],
		(uint32_t)vgl_mem_get_free_space(VGL_MEM_PHYCONT),
		(uint32_t)mempool_size[VGL_MEM_PHYCONT], (uint32_t)(uintptr_t)mempool_addr[VGL_MEM_PHYCONT],
		(uint32_t)vgl_mem_get_free_space(VGL_MEM_BUDGET),
		(uint32_t)mempool_size[VGL_MEM_BUDGET], (uint32_t)(uintptr_t)mempool_addr[VGL_MEM_BUDGET],
		(uint32_t)mempool_size[VGL_MEM_EXTERNAL], (uint32_t)(uintptr_t)mempool_addr[VGL_MEM_EXTERNAL]);
#endif
}
