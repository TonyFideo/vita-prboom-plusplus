...
void vglSetupUniformCircularPool() {
	if (!unif_pool) {
		...
		if (!unif_pool) {
			...
			vgl_log("[VGL-DIAG] uniform pool allocation failed: bytes=%u free_ram=%u free_vram=%u\n",
				(uint32_t)UNIFORM_CIRCULAR_POOL_SIZE,
...
void *vglReserveUniformCircularPoolBuffer(uint32_t size) {
	void *r;
	if (!unif_pool || size > UNIFORM_CIRCULAR_POOL_SIZE) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_alloc_failures++;
		vgl_log("[VGL-DIAG] uniform reservation failed: bytes=%u pool=%u pool_ptr=0x%08X\n",
			size, (uint32_t)UNIFORM_CIRCULAR_POOL_SIZE, (uint32_t)(uintptr_t)unif_pool);
#endif
		return NULL;
	}
	if (unif_idx > UNIFORM_CIRCULAR_POOL_SIZE - size) {
