...
/* Texture uploads use the circular pool as temporary, GPU-visible staging
 * memory.  A failed reservation used to be passed directly to memcpy or
 * sceGxmTransferCopy, turning an ordinary memory-pressure condition into an
 * access violation.  Keep the failure visible in the log and let the caller
 * abandon this texture upload safely. */
static inline __attribute__((always_inline)) void *gpu_alloc_texture_upload_temp(size_t size, const char *stage) {
	void *res = gpu_alloc_mapped_temp(size);
#ifdef VITA_GL_DIAGNOSTICS
	if (!res) {
		vgl_log("[VGL-DIAG] texture upload staging allocation failed: stage=%s bytes=%u free_ram=%u free_vram=%u\n",
			stage, (uint32_t)size,
...
void *gpu_alloc_palette(const void *data, uint32_t w, uint32_t bpe) {
	...
	if (!texture_palette) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_log("[VGL-DIAG] palette allocation failed: bytes=%u free_vram=%u free_ram=%u\n",
			256U * (uint32_t)sizeof(uint32_t),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
		return NULL;
...
void gpu_alloc_cube_texture(uint32_t w, uint32_t h, SceGxmTextureFormat format, SceGxmTransferFormat src_format, const void *data, texture *tex, uint8_t src_bpp, int index) {
	...
	if (base_texture_data) {
		...
		if (data) {
			...
			void *mapped_data = gpu_alloc_texture_upload_temp(tex_size, "cube");
			if (!mapped_data) {
				if (tex->faces_counter == 1) {
					vgl_free(base_texture_data);
					tex->data = NULL;
...
#ifdef VITA_GL_DIAGNOSTICS
		vgl_log("[VGL-DIAG] cube texture allocation failed: face=%d size=%u total=%u format=0x%08X free_vram=%u free_ram=%u\n",
			index, (uint32_t)face_size, (uint32_t)face_size * 6U, (uint32_t)format,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
		tex->data = NULL;
...
#ifdef VITA_GL_DIAGNOSTICS
		vgl_log("[VGL-DIAG] texture allocation failed: size=%u dimensions=%ux%u format=0x%08X free_vram=%u free_ram=%u\n",
			(uint32_t)tex_size, w, h, (uint32_t)format,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
		tex->data = NULL;
...
void gpu_alloc_paletted_texture(int32_t level, uint32_t w, uint32_t h, SceGxmTextureFormat format, const void *data, texture *tex, uint8_t src_bpp, uint32_t (*read_cb)(void *)) {
	...
	if (!tex->palette_data || !tex->data || (data && !read_cb)) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_log("[VGL-DIAG] paletted texture allocation failed: level=%d dimensions=%ux%u format=0x%08X palette=%p data=%p bytes=%u free_vram=%u free_ram=%u\n",
			(int)level, orig_w, orig_h, (uint32_t)format,
			tex->palette_data, tex->data, tex_size,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
		if (tex->palette_data)
...
void gpu_alloc_compressed_cube_texture(uint32_t w, uint32_t h, SceGxmTextureFormat format, uint32_t image_size, const void *data, texture *tex, uint8_t src_bpp, GLboolean uncompressed, int index) {
	...
	void *base_texture_data = tex->faces_counter == 1 ? gpu_alloc_mapped_for_gpu(face_size * 6) : tex->data;
	void *texture_data = base_texture_data ? (uint8_t *)base_texture_data + face_size * index : NULL;
			...
			if (uncompressed) {
...
#else
						void *mapped_src = gpu_alloc_texture_upload_temp(image_size, "compressed cube UBC");
						if (!mapped_src) {
							if (tex->faces_counter == 1) {
								vgl_free(base_texture_data);
								tex->data = NULL;
				...
				case SCE_GXM_TEXTURE_FORMAT_PVRTII2BPP_ABGR:
					if (aligned_width == w && aligned_height == h && h <= 2048) {
						...
						void *mapped_src = data;
#else
						void *mapped_src = gpu_alloc_texture_upload_temp(image_size, "compressed cube PVRT");
						if (!mapped_src) {
							if (tex->faces_counter == 1) {
								vgl_free(base_texture_data);
								tex->data = NULL;
				...
				default:
					if (aligned_width == w && aligned_height == h && h <= 2048) {
						...
						void *mapped_src = data;
#else
						void *mapped_src = gpu_alloc_texture_upload_temp(image_size, "compressed cube default");
						if (!mapped_src) {
							if (tex->faces_counter == 1) {
								vgl_free(base_texture_data);
								tex->data = NULL;
...
#ifdef VITA_GL_DIAGNOSTICS
		vgl_log("[VGL-DIAG] compressed cube texture allocation failed: face=%d size=%u total=%u format=0x%08X free_vram=%u free_ram=%u\n",
			index, (uint32_t)face_size, (uint32_t)face_size * 6U, (uint32_t)format,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
...
void gpu_alloc_compressed_texture(int32_t mip_level, uint32_t w, uint32_t h, SceGxmTextureFormat format, uint32_t image_size, const void *data, texture *tex, uint8_t src_bpp, GLboolean uncompressed) {
	...
	if (mip_level) {
		...
		if (mip_count >= mip_level)
			...
			if (!texture_data) {
				...
				if (!texture_data) {
#ifdef VITA_GL_DIAGNOSTICS
					vgl_log("[VGL-DIAG] compressed mipchain allocation failed: level=%d size=%u dimensions=%ux%u format=0x%08X free_vram=%u free_ram=%u\n",
						(int)mip_level, (uint32_t)tex_size, w, h, (uint32_t)format,
						(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
						(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
					return;
	...
	if (!texture_data) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_log("[VGL-DIAG] compressed texture allocation failed: level=%d size=%u dimensions=%ux%u format=0x%08X free_vram=%u free_ram=%u\n",
			(int)mip_level, (uint32_t)tex_size, w, h, (uint32_t)format,
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
			(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
			...
			if (uncompressed) {
				...
				case SCE_GXM_TEXTURE_FORMAT_UBC5_GR:
					if (aligned_width == w && aligned_height == h && h <= 2048 && w <= 2048) {
						...
						void *mapped_src = data;
#else
						void *mapped_src = gpu_alloc_texture_upload_temp(image_size, "compressed UBC");
						if (!mapped_src) {
							if (!mip_level) {
				...
				case SCE_GXM_TEXTURE_FORMAT_PVRTII2BPP_ABGR:
					if (aligned_width == w && aligned_height == h && h <= 2048) {
						...
						void *mapped_src = data;
#else
						void *mapped_src = gpu_alloc_texture_upload_temp(image_size, "compressed PVRT");
						if (!mapped_src) {
							if (!mip_level) {
								vgl_free(texture_data);
								tex->data = NULL;
				...
				default:
					if (aligned_width == w && aligned_height == h && h <= 2048) {
						...
						void *mapped_src = data;
#else
						void *mapped_src = gpu_alloc_texture_upload_temp(image_size, "compressed default");
						if (!mapped_src) {
							if (!mip_level) {
								vgl_free(texture_data);
								tex->data = NULL;
...
void gpu_alloc_mipmaps(int level, texture *tex) {
	...
	if ((level > count) || (level < 0)) { // Note: level < 0 means we will use max possible mipmaps level
		...
		if (count <= 0 && !texture_data) {
			...
			if (!texture_data) {
#ifdef VITA_GL_DIAGNOSTICS
				vgl_log("[VGL-DIAG] mipmap allocation failed: levels=%d size=%u dimensions=%ux%u format=0x%08X free_vram=%u free_ram=%u\n",
					level, size, orig_w, orig_h, (uint32_t)format,
					(uint32_t)vgl_mem_get_free_space(VGL_MEM_VRAM),
					(uint32_t)vgl_mem_get_free_space(VGL_MEM_RAM));
#endif
				return;
		...
		if (!texture_data) {
#ifdef VITA_GL_DIAGNOSTICS
			vgl_log("[VGL-DIAG] mipmap data unavailable: levels=%d dimensions=%ux%u format=0x%08X\n",
				level, orig_w, orig_h, (uint32_t)format);
#endif
			return;
		}
