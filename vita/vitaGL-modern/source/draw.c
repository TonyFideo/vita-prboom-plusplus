...
void vglDrawObjects(GLenum mode, GLsizei count) {
	...
	if (!gxm_context || !index_object || !vertex_object ||
		((ffp_vertex_attrib_state & (1 << FFP_ATTRIB_TEX0)) && !texture_object) ||
		((ffp_vertex_attrib_state & (1 << FFP_ATTRIB_COLOR)) && !color_object)) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_alloc_failures++;
		vgl_diag_frame_dropped_draws++;
		vgl_log("[VGL-DIAG] vglDrawObjects skipped before FFP/custom draw: mode=%u count=%d context=0x%08X vertex=0x%08X texture=0x%08X color=0x%08X index=0x%08X state=0x%04X\n",
			(uint32_t)mode, count,
			(uint32_t)(uintptr_t)gxm_context,
			(uint32_t)(uintptr_t)vertex_object,
			(uint32_t)(uintptr_t)texture_object,
			(uint32_t)(uintptr_t)color_object,
			(uint32_t)(uintptr_t)index_object,
			(uint32_t)ffp_vertex_attrib_state);
#endif
		restore_polygon_mode(gxm_p);
		return;
	}
	if (cur_program != 0) {
		...
		int draw_result = sceGxmDraw(gxm_context, gxm_p, SCE_GXM_INDEX_FORMAT_U16, index_object, count);
#ifdef VITA_GL_DIAGNOSTICS
		if (draw_result) {
			vgl_diag_frame_gxm_errors++;
			vgl_diag_frame_last_gxm_error = (uint32_t)draw_result;
			vgl_log("[VGL-DIAG] vglDrawObjects custom sceGxmDraw failed: result=0x%08X (%s)\n",
				(uint32_t)draw_result, get_gxm_error_literal(draw_result));
		}
#endif
		...
		if (vgl_diag_ffp_failed) {
			vgl_diag_frame_dropped_draws++;
			vgl_log("[VGL-DIAG] vglDrawObjects skipped after FFP failure: frame=%u\n", vgl_diag_frame_serial);
			restore_polygon_mode(gxm_p);
			return;
		}
#endif
		if (ffp_vertex_attrib_state & (1 << FFP_ATTRIB_TEX0)) {
			...
			#ifdef VITA_GL_DIAGNOSTICS
				vgl_diag_frame_dropped_draws++;
				vgl_log("[VGL-DIAG] vglDrawObjects skipped invalid texture: unit=0 tex_id=%u status=%u frame=%u\n",
					(uint32_t)tex_unit->tex_id[0],
					(uint32_t)texture_slots[tex_unit->tex_id[0]].status,
					vgl_diag_frame_serial);
			#endif
				restore_polygon_mode(gxm_p);
				return;
...
#endif
			int texture_result = sceGxmSetFragmentTexture(gxm_context, 0, &texture_slots[tex_unit->tex_id[0]].gxm_tex);
			#ifdef VITA_GL_DIAGNOSTICS
			if (texture_result) {
				vgl_diag_frame_gxm_errors++;
				vgl_diag_frame_last_gxm_error = (uint32_t)texture_result;
				vgl_log("[VGL-DIAG] vglDrawObjects fragment texture failed: tex_id=%u result=0x%08X (%s)\n",
					(uint32_t)tex_unit->tex_id[0], (uint32_t)texture_result,
					get_gxm_error_literal(texture_result));
				restore_polygon_mode(gxm_p);
				return;
			}
			#endif
			int stream_result = sceGxmSetVertexStream(gxm_context, 1, texture_object);
			#ifdef VITA_GL_DIAGNOSTICS
			if (stream_result) {
				vgl_diag_frame_stream_errors++;
				vgl_diag_frame_last_stream_error = (uint32_t)stream_result;
				vgl_log("[VGL-DIAG] vglDrawObjects texture stream failed: result=0x%08X (%s)\n",
					(uint32_t)stream_result, get_gxm_error_literal(stream_result));
			}
			#endif
			if (ffp_vertex_num_params > 2) {
				int color_stream_result = sceGxmSetVertexStream(gxm_context, 2, color_object);
				#ifdef VITA_GL_DIAGNOSTICS
				if (color_stream_result) {
					vgl_diag_frame_stream_errors++;
					vgl_diag_frame_last_stream_error = (uint32_t)color_stream_result;
					vgl_log("[VGL-DIAG] vglDrawObjects color stream failed: result=0x%08X (%s)\n",
						(uint32_t)color_stream_result, get_gxm_error_literal(color_stream_result));
					restore_polygon_mode(gxm_p);
					return;
				}
				#endif
			...
			int color_stream_result = sceGxmSetVertexStream(gxm_context, 1, color_object);
			#ifdef VITA_GL_DIAGNOSTICS
			if (color_stream_result) {
				vgl_diag_frame_stream_errors++;
				vgl_diag_frame_last_stream_error = (uint32_t)color_stream_result;
		...
		int stream_result = sceGxmSetVertexStream(gxm_context, 0, vertex_object);
		#ifdef VITA_GL_DIAGNOSTICS
		if (stream_result) {
			vgl_diag_frame_stream_errors++;
			vgl_diag_frame_last_stream_error = (uint32_t)stream_result;
			vgl_log("[VGL-DIAG] vglDrawObjects vertex stream failed: result=0x%08X (%s)\n",
				(uint32_t)stream_result, get_gxm_error_literal(stream_result));
		}
		#endif
		int draw_result = sceGxmDraw(gxm_context, gxm_p, SCE_GXM_INDEX_FORMAT_U16, index_object, count);
		...
		if (draw_result) {
			vgl_diag_frame_gxm_errors++;
			vgl_diag_frame_last_gxm_error = (uint32_t)draw_result;
			vgl_log("[VGL-DIAG] vglDrawObjects sceGxmDraw failed: mode=%u count=%d result=0x%08X (%s)\n",
				(uint32_t)mode, count, (uint32_t)draw_result, get_gxm_error_literal(draw_result));
		}
		#endif
	}
