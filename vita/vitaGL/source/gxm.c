...
void beginGxmScene(void) {
	...
	if (active_write_fb == NULL) { // Default framebuffer is used
		result = sceGxmBeginScene(gxm_context, gxm_scene_flags, gxm_render_target,
			NULL, NULL,
		...
		result = sceGxmBeginScene(gxm_context, gxm_scene_flags, active_write_fb->target,
			NULL, NULL, NULL,
	...
	// Do not claim that a scene is active when GXM rejected it.  Continuing
	// into sceGxmDraw would produce NOT_WITHIN_SCENE errors and may stall the
	// command stream on hardware.
	if (result != 0) {
