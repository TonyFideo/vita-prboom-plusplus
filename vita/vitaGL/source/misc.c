...
void glReadPixels(GLint x, GLint y, GLsizei width, GLsizei height, GLenum format, GLenum type, GLvoid *data) {
	...
	/*
	 * The display framebuffer is not available until VitaGL has submitted a
	 * frame.  sceDisplayGetFrameBuf() can therefore return a successful-looking
	 * structure with a NULL base during early startup (and Vita3K exposes this
	 * case).  The old implementation converted that NULL into a read from
	 * address 0.  Keep the caller's buffer usable, but do not touch the display
	 * buffer until it has a valid address and pitch.
	 */
	if (width < 0 || height < 0) {
		SET_GL_ERROR(GL_INVALID_VALUE)
	}
	...
	bytes_per_pixel = (format == GL_RGBA) ? 4 : 3;

	memset(&pParam, 0, sizeof(pParam));
	pParam.size = sizeof(SceDisplayFrameBuf);
	if (sceDisplayGetFrameBuf(&pParam, SCE_DISPLAY_SETBUF_NEXTFRAME) < 0 ||
		!pParam.base || pParam.pitch == 0) {
		memset(data, 0, (size_t)width * (size_t)height * (size_t)bytes_per_pixel);
		return;
	}
