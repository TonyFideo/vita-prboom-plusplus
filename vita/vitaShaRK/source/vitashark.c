...
SceGxmProgram *shark_compile_shader_extended(const char *src, uint32_t *size, shark_type type, shark_opt opt, int32_t use_fastmath, int32_t use_fastprecision, int32_t use_fastint) {
	...
	/* sceShaccCg can fail without returning a compile-output object.  The
	 * previous code dereferenced that NULL result while trying to enumerate
	 * diagnostics, which turns a shader-compile error into an unexplained
	 * access violation in the game. */
	if (!shark_output) {
		if (shark_log_cb)
			shark_log_cb("sceShaccCgCompileProgram returned no compile output", SHARK_LOG_ERROR, -1);
		return NULL;
	}
	// Executing logging
	if (shark_log_cb) {
	...
	if (!shark_output->programData) {
		if (shark_log_cb)
			shark_log_cb("sceShaccCg returned no program data", SHARK_LOG_ERROR, -1);
		return NULL;
	}
